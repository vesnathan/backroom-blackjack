...
export const handler: PreSignUpTriggerHandler = async (
  ...
  if (triggerSource === "PreSignUp_SignUp") {
    try {
      ...
      console.log(
        "Checking for external provider accounts:",
        JSON.stringify(existingUsers.Users, null, 2),
      );

      // Find external provider user (Google)
      ...
      if (googleUser) {
        console.log("Found existing Google user:", googleUser.Username);
  ...
  // This is an external provider sign-up (Google)
