/**
 * Bootstrap Resource Checker for Backroom Blackjack
 *
 ...
 *
 * The deployment user (backroom-blackjack-deploy) has MINIMAL permissions:
 *   - Create/update CloudFormation stacks (scoped to backroom-blackjack-* stacks only)
...
You need access keys for the backroom-blackjack-deploy IAM user.
If you haven't created this user yet, do that first:

  1. Go to IAM > Users > Create user
  2. User name: backroom-blackjack-deploy
  3. Create user (no console access needed)
  4. Add inline policy from: deploy/iam-policies/backroom-blackjack-deploy-policy.json
...
const DEPLOY_USER_NAME = "backroom-blackjack-deploy";

export function getBootstrapConfig(_stage: string): BootstrapConfig {
  // Use CFN_ROLE_ARN from .env if available
  const cfnRoleArn = process.env.CFN_ROLE_ARN || `arn:aws:iam::${ACCOUNT_ID}:role/backroom-blackjack-cfn-role`;
  const cfnRoleName = cfnRoleArn.split('/').pop() || 'backroom-blackjack-cfn-role';

  return {
    templateBucketName: "backroom-blackjack-deploy-templates",
    cfnRoleName,
    cfnRoleArn,
...
export async function checkBootstrapResources(stage: string): Promise<BootstrapCheckResult> {
  const config = getBootstrapConfig(stage);
...
`);
  ...
  if (missingUser) {
    const policyJson = JSON.stringify({
      "Version": "2012-10-17",
      "Statement": [
        {"Sid": "CloudFormation", "Effect": "Allow", "Action": ["cloudformation:CreateStack", "cloudformation:UpdateStack", "cloudformation:DeleteStack", "cloudformation:DescribeStacks", "cloudformation:DescribeStackEvents", "cloudformation:DescribeStackResources", "cloudformation:GetTemplate"], "Resource": "arn:aws:cloudformation:*:430118819356:stack/backroom-blackjack-*/*"},
        {"Sid": "CloudFormationList", "Effect": "Allow", "Action": "cloudformation:ListStacks", "Resource": "*"},
        {"Sid": "S3BucketObjects", "Effect": "Allow", "Action": ["s3:PutObject", "s3:GetObject", "s3:DeleteObject"], "Resource": ["arn:aws:s3:::backroom-blackjack-deploy-templates/*", "arn:aws:s3:::backroom-blackjack-frontend-*/*"]},
        {"Sid": "S3BucketList", "Effect": "Allow", "Action": ["s3:ListBucket", "s3:GetBucketLocation"], "Resource": ["arn:aws:s3:::backroom-blackjack-deploy-templates", "arn:aws:s3:::backroom-blackjack-frontend-*"]},
        {"Sid": "CloudFrontInvalidation", "Effect": "Allow", "Action": ["cloudfront:CreateInvalidation", "cloudfront:GetInvalidation"], "Resource": "*"},
        {"Sid": "PassCFNRole", "Effect": "Allow", "Action": "iam:PassRole", "Resource": "arn:aws:iam::430118819356:role/backroom-blackjack-cfn-role", "Condition": {"StringEquals": {"iam:PassedToService": "cloudformation.amazonaws.com"}}},
        {"Sid": "AssumeDeployRoles", "Effect": "Allow", "Action": "sts:AssumeRole", "Resource": "arn:aws:iam::430118819356:role/backroom-blackjack-*-deploy-role-*"},
        {"Sid": "IAMReadRole", "Effect": "Allow", "Action": "iam:GetRole", "Resource": "arn:aws:iam::430118819356:role/backroom-blackjack-cfn-role"},
        {"Sid": "IAMReadSelf", "Effect": "Allow", "Action": "iam:GetUser", "Resource": "arn:aws:iam::430118819356:user/backroom-blackjack-deploy"}
      ]
    });
